...
title: "Bioinformatics Workflow Template: Standardizing CI/CD Across Python, Nextflow, and Snakemake"
authors: [river]
tags: [ci-cd, bioinformatics, python, nextflow, snakemake, docker, testing, template]
image: ./imgs/intro.png
---

Building reproducible bioinformatics pipelines is hard. Every project starts from scratch with its own testing, CI/CD, and deployment strategy. **What if you could clone a template, change the analysis tool, and be ready to go?**

This post introduces a **standardized bioinformatics workflow template** designed to work across multiple frameworks (Python, Nextflow, Snakemake) with consistent testing, CI/CD, and project structure. Based on real production use at `/home/giangnguyen/Documents/dev/docs/bioinfor-wf-template`, this template reduces setup time from days to minutes while maintaining research reproducibility.
...
- ✅ Multi-framework support (Python, Nextflow, Snakemake)
...
## Part 4: Testing Different Frameworks

The template is designed to work with any bioinformatics framework. Here's how:

### Python Tools (Current)

Already implemented with apps like `fastqc`, `multiqc`.
...
### Nextflow Workflows (Template Ready)

```bash
# workflows/alignment_nf/main.nf
process fastqc {
    input:
        tuple val(sample_id), path(fastq1), path(fastq2)
    output:
        path "fastqc_results/*.html"
    script:
        """
        fastqc ${fastq1} ${fastq2} -o fastqc_results/
        """
}
...
process bowtie2 {
    ...
    script:
        """
        bowtie2-build ${params.reference} index
        bowtie2 -x index -1 ${fastq1} -2 ${fastq2} -S output.bam
        """
}

workflow alignment {
    fastqc_ch = fastqc(input_ch)
    bam_ch = bowtie2(input_ch)
}
```

**Testing Nextflow with template:**
```bash
# workflows/alignment_nf/tests/test_e2e.py
def test_nextflow_alignment_e2e():
    """Run Nextflow workflow end-to-end."""
    cmd = ["nextflow", "run", "main.nf", "-with-docker"]
    result = subprocess.run(cmd, cwd="workflows/alignment_nf", check=True)
    assert Path("results/bam").exists()

# workflows/alignment_nf/Makefile
e2e:
	pytest -v -k "e2e" tests/
```

### Snakemake Pipelines (Template Ready)

```bash
# workflows/variant_calling/Snakefile
rule fastqc:
    input:
        fastq1="{sample}_R1.fastq.gz",
        fastq2="{sample}_R2.fastq.gz"
    output:
        html=directory("results/fastqc/{sample}")
    shell:
        """
        fastqc {input.fastq1} {input.fastq2} -o {output.html}
        """
...
rule align:
    ...
    shell:
        """
        bowtie2-build {input.ref} index
        bowtie2 -x index -1 {input.fastq1} -2 {input.fastq2} -S {output.bam}
        """
```

**Testing Snakemake with template:**
```bash
# workflows/variant_calling/tests/test_e2e.py
def test_snakemake_workflow_e2e():
    """Run Snakemake workflow end-to-end."""
    cmd = [
        "snakemake",
        "--cores", "4",
        "--use-conda",
    ...
    result = subprocess.run(cmd, cwd="workflows/variant_calling", check=True)
    assert Path("results/bam").exists()

# workflows/variant_calling/Makefile
e2e:
	pytest -v -k "e2e" tests/
```
...
| **Multi-framework support** | Not possible | Python, Nextflow, Snakemake all supported |
